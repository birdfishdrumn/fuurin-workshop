...
const Favorite: React.FC<Partial<FAVORITE>> = ({ id, uid, likesPostsArray, post, detail }) => {
  ...
  const addToFavorite = useCallback(
    (event: any) => {
        ...
        return db
          ...
          .then((querySnapshot) => {
            querySnapshot.forEach((doc) => {
              ...
              return db.collection('users').doc(uid).collection('likes').doc(dataId).delete();
            });
          })
          .then(() => {
            db.collection('posts')
      ...
      detail &&
        dispatch(dialogOpenAction({ type: 'favoriteAction', title: '作品をお気に入りに登録' }));
      // }
    },
    [likesPostsArray]
  );
        ...
        {isSignedIn && (
          <>
            <IconButton onClick={addToFavorite} style={{ paddingRight: '0' }}>
              ...
              <FavoriteCount>
                {' '}
                {post.uid === uid && post.likes ? post.likes.length : <></>}
              </FavoriteCount>
            </IconButton>
          </>
        )}
