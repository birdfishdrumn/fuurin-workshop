...
const Comment: React.FC<PROPS> = memo(({ id, avatar, username, uid, postUid }) => {
  ...
  useEffect(() => {
    if (id) {
      const unSub = comRef.orderBy('timestamp', 'desc').onSnapshot((snapshot) => {
        setComments(
          snapshot.docs.map(
            (doc) =>
              ({
  ...
  return (
    <div>
      <form onSubmit={newComment}>
        {isSignedIn && <CommentForm avatar={avatar} setComment={setComment} comment={comment} />}
      </form>

      <PostCommentLength>
        <PostCommentIcon onClick={() => setOpen(!open)} />
        <StyledBoldText center>
          {comments.length ? (
            <>{comments.length}件のコメントがあります</>
          ) : (
            'コメントがまだありません'
          )}
        </StyledBoldText>
              ...
              <PostComment key={com.id}>
                ...
                {(postUid === uid || com.reply === true) && (
                  <>
                    <Accordion
                      square
                      expanded={expanded === com.text}
                      onChange={handleChange(com.text)}
                      onClick={fetchReplyComment(com.id)}
                    >
