  ...
  return (
    <ShareWrapper>
      ...
      <Menu
        ...
        onClose={handleClose}
      >
        {type === 'share' && <Share url={url} handleClose={handleClose} />}
        {type === 'comment' && (
          <MenuItem onClick={() => deleteComment(id)}>コメントを削除</MenuItem>
        )}
        {type === 'report' && (
          <MenuItem
            onClick={() =>
              dispatch(
                dialogOpenAction({ type: 'report', title: '違反の報告', id: id, content: content })
              )
            }
          >
            <FlagIcon />
