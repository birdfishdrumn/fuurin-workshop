    ...
    <SectionWrapper top>
      <ProfileColumn>
        <div>
          ...
          {isSignedIn && myUserId !== uid && (
            <Button
              startIcon={
                favoriteUserArray && favoriteUserArray.includes(uid) ? (
                  <CheckIcon />
                ) : (
                  <FavoriteIcon />
                )
              ...
              variant={
                favoriteUserArray && favoriteUserArray.includes(uid) ? 'contained' : 'outlined'
              }
        ...
        <div>
          ...
          {profile ? (
            <MinText left>{returnCodeToBr(profile)}</MinText>
          ) : (
            'プロフィールが記述されていません。'
          )}
        </div>
      ...
      {!posts.length && (
        <div className="center">
          ...
          <BoldText color={'dimgray'}>投稿はまだありません</BoldText>
        </div>
      )}
    </SectionWrapper>
  );
