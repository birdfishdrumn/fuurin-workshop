...
const WorkshopCaution = () => {
  ...
  return (
    <SectionWrapping>
      <Title>色の作り方</Title>
      <Text left>
        こちらでは色の作り方を紹介します。配色リストの絵をクリックすると、その柄で使う色を確認することができます。
      </Text>
      ...
      <BoldText id="list">
        <ForwardIcon style={{ fontSize: '40px', marginBottom: '-10px', marginRight: '10px' }} />
        右にスライドできます
      </BoldText>
      ...
      <SliderWrapper>
        <ColorBackGround>
          ...
          <ColorWrapper>
            {colors.length ? (
              colors.slice(0, 10).map((color) => <ColorContainerBox key={color.id} color={color} />)
            ) : (
              <CircularProgress
                color="inherit"
                style={{ margin: '50px auto !important', width: '20px !important' }}
              />
            )}
          </ColorWrapper>
        </ColorBackGround>
          ...
          <ColorBackGround>
            <ColorWrapper>
              {colors.length ? (
                colors
                  .slice(10, 20)
                  .map((color) => <ColorContainerBox key={color.id} color={color} />)
              ) : (
                <CircularProgress
                  color="inherit"
                  style={{ margin: '50px auto !important', width: '20px !important' }}
