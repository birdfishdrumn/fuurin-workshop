...
exports.onUpdateUser = functions
  ...
  .onUpdate(async (change: any, context: any) => {
    ...
    try {
      const snapshot = await db.collection('posts').where('uid', '==', userId).get();
      ...
      const commentSnapshot = await db.collectionGroup('comments').where('id', '==', userId).get();
      ...
      const replySnapshot = await db.collectionGroup('reply').where('uid', '==', userId).get();
...
exports.onDeleteUser = functions
  ...
  .onDelete(async (snap: any, context: any) => {
    ...
    try {
      // お気に入りの作品を削除する。
      await firebaseTools.firestore.delete(`users/${userId}/likes`, {
      ...
      await firebaseTools.firestore.delete(`users/${userId}/likeUser`, {
        project: process.env.GCLOUD_PROJECT,
        recursive: true,
        yes: true,
        token: functions.config().fb.token,
      ...
      const db = admin.firestore();
      const batch = db.batch();
      const likeUser = await db.collectionGroup('likeUser').where('uid', '==', userId).get();

      likeUser.docs.forEach((doc: any) => {
        batch.delete(doc.ref);
      });
      // 投稿した作品を全て削除
      const postSnapshot = await db.collection('posts').where('uid', '==', userId).get();

      postSnapshot.docs.forEach((postDoc: any) => {
        batch.delete(postDoc.ref);
      });

      const reply = await db.collectionGroup('reply').where('uid', '==', userId).get();
...
exports.onDeletePost = functions
  ...
  .onDelete(async (snap: any, context: any) => {
    ...
    const batch = db.batch();
    try {
      await firebaseTools.firestore.delete(`posts/${postId}/comments`, {
        project: process.env.GCLOUD_PROJECT,
        recursive: true,
        yes: true,
        token: functions.config().fb.token,
      ...
      const likes = await db.collectionGroup('likes').where('postId', '==', postId).get();
